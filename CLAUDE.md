...
iteration. `Order.id` is generated by `DEFAULT uuidv7()` in PostgreSQL and surfaced via `RETURNING id`; `Submit` returns
this id so the handler can pass it to Monobank as `merchantPaymInfo.reference` and to `AttachInvoice`. The Monobank
