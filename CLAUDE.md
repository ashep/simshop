...
filename. `product.Product.AttrImages` stores the raw map; `product.ProductDetail.AttrImages` holds the rewritten
form (`attr_key → value_key → URL path`) built by `ServeProductContent`. The path-rewriting logic follows the same
pattern as `p.Images`: bare filenames become `/images/{id}/{filename}`. Unlike `attr_prices`, no fallback to
`"default"` is needed because there is no country dimension.
